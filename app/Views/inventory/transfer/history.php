...
$sourceWarehouse = (int) ($filters['source_warehouse'] ?? 0);
$destinationWarehouse = (int) ($filters['destination_warehouse'] ?? 0);
...
<div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
    <div>
        <h2 class="text-2xl font-bold text-slate-800">Transfer History</h2>
        <p class="mt-1 text-sm text-slate-500">
            Filter transfer records by item, source warehouse, destination warehouse, and transfer date.
        </p>
    </div>
...
<form method="GET" action="/inventory/transfer/history" class="mb-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            >
                ...
                <?php foreach ($warehouses as $warehouse): ?>
                    <option
                        ...
                        <?= $sourceWarehouse === (int) ($warehouse['id'] ?? 0) ? 'selected' : '' ?>
                    >
        ...
        <div>
            ...
            >
                ...
                <?php foreach ($warehouses as $warehouse): ?>
                    <option
                        value="<?= (int) ($warehouse['id'] ?? 0) ?>"
                        <?= $destinationWarehouse === (int) ($warehouse['id'] ?? 0) ? 'selected' : '' ?>
                    >
                        <?= htmlspecialchars((string) ($warehouse['name'] ?? '')) ?>
                    </option>
                <?php endforeach; ?>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600">Transfer ID</th>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600">Item Name</th>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600">Source Warehouse</th>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600">Source Pallet</th>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600">Destination Warehouse</th>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600">Destination Pallet</th>
                        <th class="px-4 py-3 text-right font-semibold text-slate-600">Crates Transferred</th>
                        <th class="px-4 py-3 text-right font-semibold text-slate-600">Pieces Transferred</th>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600">Date Transferred</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php foreach ($records as $row): ?>
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 text-slate-700">
                                <?= htmlspecialchars((string) ($row['source_warehouse_name'] ?? '')) ?>
                            </td>
                            <td class="px-4 py-3 text-slate-700">
                                <?= htmlspecialchars((string) ($row['source_pallet'] ?? '')) ?>
                            </td>
                            <td class="px-4 py-3 text-slate-700">
                                <?= htmlspecialchars((string) ($row['destination_warehouse_name'] ?? '')) ?>
                            </td>
                            <td class="px-4 py-3 text-slate-700">
                                <?= htmlspecialchars((string) ($row['dest_pallet'] ?? '')) ?>
                            </td>
                            <td class="px-4 py-3 text-right text-slate-700">
                                <?= number_format((int) ($row['quantity_transferred'] ?? 0)) ?>
                            </td>
                            <td class="px-4 py-3 text-right text-slate-700">
                                <?= number_format((int) ($row['pieces_transferred'] ?? 0)) ?>
                            </td>
                            <td class="px-4 py-3 text-slate-700">
                                <?= htmlspecialchars((string) ($row['date_transferred'] ?? '')) ?>
                            </td>
                            <td class="px-4 py-3 text-slate-700">
                            ...
                            <td class="px-4 py-3">
                                <a
                                    href="/inventory/transfer/edit?id=<?= (int) ($row['id'] ?? 0) ?>"
