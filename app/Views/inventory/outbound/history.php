...
<div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
    <div>
        <h2 class="text-2xl font-bold text-slate-800">Outbound Inventory History</h2>
        <p class="mt-1 text-sm text-slate-500">
            Filter outbound records by item, warehouse, and date removed.
        </p>
    </div>
...
<form method="GET" action="/inventory/outbound/history" class="mb-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            >
                ...
                <?php foreach ($warehouses as $warehouse): ?>
                    <option
                        ...
                        <?= $warehouseId === (int) ($warehouse['id'] ?? 0) ? 'selected' : '' ?>
                    >
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right font-semibold text-slate-600">Crates Removed</th>
                        <th class="px-4 py-3 text-right font-semibold text-slate-600">Pieces Removed</th>
                        ...
                        <th class="px-4 py-3 text-left font-semibold text-slate-600">Outbound Type</th>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600">Date Removed</th>
                    ...
                    <?php foreach ($records as $row): ?>
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 text-right text-slate-700">
                                <?= number_format((int) ($row['quantity_removed'] ?? 0)) ?>
                            </td>
                            <td class="px-4 py-3 text-right text-slate-700">
                            ...
                            <td class="px-4 py-3 text-slate-700">
                                <?= htmlspecialchars((string) ($row['outbound_type'] ?? '')) ?>
                            </td>
                            <td class="px-4 py-3 text-slate-700">
                                <?= htmlspecialchars((string) ($row['date_removed'] ?? '')) ?>
                            </td>
                            <td class="px-4 py-3 text-slate-700">
                            ...
                            <td class="px-4 py-3">
                                <a
                                    href="/inventory/outbound/edit?id=<?= (int) ($row['id'] ?? 0) ?>"
